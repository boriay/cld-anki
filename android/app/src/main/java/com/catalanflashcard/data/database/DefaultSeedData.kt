...
/**
 * The default decks/cards shipped to a brand-new local store: one Catalan deck
 * per UI language (shared fronts, per-language backs). Single source of truth so
 * both seed paths agree — [InitialDataCallback] (first DB creation, runs offline)
 * and [LocalSeeder] (re-seed after a sign-out wipe resets to a fresh anonymous
 * account). The backend keeps an equivalent generated copy (seed/data.go).
 */
object DefaultSeedData {

    /** One default deck, named in its own language and tagged with that language. */
    data class SeedDeck(val language: String, val name: String)

    /** Catalan front plus its translation in each supported language. */
    data class SeedCard(
        val front: String,
        val en: String,
        val es: String,
